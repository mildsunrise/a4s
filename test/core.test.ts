...
describe('Core signing', () => {
    ...
    it('simple cache works correctly', () => {
        ...
        const key1 = {
            args: ['20310430', 'test', 'eu-west-3', 'service'],
            result: {
                scope: '20310430/eu-west-3/service/aws4_request',
                key: Buffer.from('674f7e8745f37c14fe188810745a0c54d78824d747e69eb74396c369ae5f7b4d', 'hex')
            }
        ...
        const key2 = {
            args: ['20310431', 'test', 'eu-west-3', 'service'],
            result: {
        ...
        expect(derive(...key1.args)).toStrictEqual(key1.result)
        expect(derive(...key1.args)).toStrictEqual(key1.result)
        expect(derive(...key2.args)).toStrictEqual(key2.result)
        expect(derive(...key1.args)).toStrictEqual(key1.result)
        expect(derive(...key2.args)).toStrictEqual(key2.result)
        expect(derive(...key2.args)).toStrictEqual(key2.result)
    })
